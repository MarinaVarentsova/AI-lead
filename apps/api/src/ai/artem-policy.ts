...
export function fallbackReply(markdown: string, program: ArtemProgram, question: string,
  ...
  if (/что входит|содержан|есть практика|практическ.*(?:задани|работ)|что (?:я )?получу после обуч/.test(q)) {
  ...
  if (/нача(?:ть|л|ло|ну)|формат|дистанц|срок|нет времени/.test(q) && program === "construction_expertise") return "«Стройэксперт» проходит дистанционно, в индивидуальном графике. Есть задания, контроль знаний и итоговая работа. Точную продолжительность и нагрузку по тарифу нужно уточнить.";
