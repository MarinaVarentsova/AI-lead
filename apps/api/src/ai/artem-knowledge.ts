...
export const ARTEM_SOURCE = "artem_unified_knowledge_base_v3_2.md";
export async function loadArtemKnowledge(moduleUrl = import.meta.url): Promise<string> {
  ...
  for (const candidate of [new URL(`./knowledge/${ARTEM_SOURCE}`, moduleUrl),
    ...
    try {
      ...
      if (!text.includes("Версия 3.2 · 17 сентября 2026 года.")) throw new Error("ARTEM_KNOWLEDGE_VERSION_INVALID");
      return text;
    } catch (error) {
