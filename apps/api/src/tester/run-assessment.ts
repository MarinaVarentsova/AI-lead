...
Оценивай по v3.2: четыре обязательных вопроса, затем условный или определённый вывод с двумя фактами и пользой; статус currently_studying требует условного вывода, а не отказа. Известный факт должен идти до CTA, текущая программа не должна теряться. Не требуй CTA после отказа от контакта и повторного профиля в каждом ответе. Явная цель важнее страницы входа.
...
const layers: Record<Area, string> = {
  knowledge_base: "knowledge/inobr/artem_unified_knowledge_base_v3_2.md",
  behavior_instruction: "knowledge/inobr/artem_unified_knowledge_base_v3_2.md — поведенческие разделы",
