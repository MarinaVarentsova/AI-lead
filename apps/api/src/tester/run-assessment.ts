...
Оценивай по v4.0: четыре обязательных вопроса, затем условный или определённый вывод с двумя фактами и пользой; статус currently_studying требует условного вывода, а не отказа. Известный факт должен идти до CTA, текущая программа не должна теряться. Small talk и off-topic не являются unknown fact и не требуют CTA; мат с содержательным вопросом оценивай по ответу на вопрос. Не требуй CTA после отказа от контакта и повторного профиля в каждом ответе. Явная цель важнее страницы входа.
...
const layers: Record<Area, string> = {
  knowledge_base: "knowledge/inobr/artem_unified_knowledge_base_v4_0.md",
  behavior_instruction: "knowledge/inobr/artem_unified_knowledge_base_v4_0.md — поведенческие разделы",
