...
Используй только persona, answers, diagnosticResult, transcript и утверждённую единую KB v3.6.
...
Учебные задания, итоговая работа, проверка преподавателем, исправление замечаний, материалы и примеры не являются подтверждением реального опыта, реальных объектов/кейсов или выполнения экспертных работ под контролем преподавателя. Такие заявления считай hallucination.
...
personalization: первая рекомендация содержит 2–3 подтверждённых факта (не механический список всех четырёх), хотя бы один из них превращён в конкретную пользу, и один следующий шаг; обычно это 2–4 предложения; в вопросе о профессии использует подтверждённую роль или сферу;
...
известный факт сначала: если цена, документ, рассрочка, профессиональная польза или правило уже есть в KB v3.6, Артём сначала отвечает по нему и не заменяет ответ отправкой к менеджеру;
...
cta: уместный переход с конкретной причиной обращения, без механического повторения; пустое «для уточнения программы и условий» недостаточно. CTA не обязан присутствовать в каждом ответе, и отсутствие CTA само по себе не ошибка; после отказа от контакта отсутствие CTA правильно, не штрафуй.
