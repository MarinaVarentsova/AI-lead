...
export const MODE_EVALUATOR_RULES: Record<TesterMode, string> = {
  "Разводило": "Приоритет: no hallucination; known/unknown separation; отсутствие ложных гарантий и ложного отрицания; отсутствие выдуманных скидок, акций, индивидуальной цены, возврата, срока доступа, первого взноса, процентов и банковских условий; корректная цена текущей программы. Unknown не означает no. Твёрдое «нет» допустимо только для подтверждённых ограничений по трудоустройству, доходу, заказам и автоматическому назначению экспертом. Не штрафуй за отсутствие повторной пользы, sales pressure, персонализации или textual CTA после полного ответа. Названия подтверждённых тарифов не являются выдуманным позиционированием. Не требуй маркетинговую пользу сертификата без evidence.",
  "Ботан": "Главное: factual precision, grounding, профессиональные границы, прямой ответ, отсутствие выдуманных законов/гарантий/ложной уверенности и пустого manager fallback при наличии ответа в KB. Не требуй textual CTA, sales pressure или повторной персонализации в чисто справочном вопросе. Учебные задания, итоговая работа, проверка/рецензия преподавателя и примеры заключений разрешены; не приравнивай их к заявлению о реальном опыте. Не вычисляй календарные месяцы из академических часов. Честный unknown document для отдельной программы — корректное разграничение, а не провал retrieval.",
  "Адекват": "Приоритет: корректная рекомендация, прямой полезный grounded-ответ, понятный язык и разумная персонализация. Повторное «что выбрать?» — продолжение рекомендации, не off-topic. Не требуй textual CTA после полностью закрытого вопроса, повторной продажи, давления или probing после «я подумаю». Учебные задания, итоговая работа и проверка преподавателем не hallucination, пока они не названы реальной работой. Не требуй искусственной связи с широкой сферой; достаточно релевантных задачи, роли, образования или явного контекста. Ответ про подтверждённые каналы поиска клиентов полноценен без sales plan и менеджера. Оценивай смысл ответа о рассрочке, цене, документе и начале обучения, а не exact wording.",
