...
export async function loadGetCourseManagerWidget(input: { sessionId: string; managerFormRequestId: string; sourceUrl: string; referrer: string },
  ...
  const bridge = `<style>[data-id="${GETCOURSE_COMMENT_FIELD}"]{display:none!important}.__artem-form-error{display:none;background:#cf5b5b;color:#fff;padding:14px 16px;margin:0 0 12px;border-radius:0;font:16px/1.4 Arial,sans-serif;text-align:left}.__artem-form-error[aria-hidden="false"]{display:block}</style><script>
window.addEventListener("load",function(){setTimeout(function(){var form=document.querySelector('form[data-id="${GETCOURSE_BLOCK_ID}"]');if(form){var action=document.createElement("input");action.type="hidden";action.name=${JSON.stringify(GETCOURSE_ACTION_FIELD)};action.value=${JSON.stringify(upstreamAction)};form.appendChild(action);var requestId=document.createElement("input");requestId.type="hidden";requestId.name=${JSON.stringify(MANAGER_FORM_REQUEST_ID_FIELD)};requestId.value=${JSON.stringify(input.managerFormRequestId)};form.appendChild(requestId);var canonicalPhone=document.createElement("input");canonicalPhone.type="hidden";canonicalPhone.name=${JSON.stringify(GETCOURSE_CANONICAL_PHONE_FIELD)};form.appendChild(canonicalPhone);var visiblePhone=form.querySelector("input.iti__tel-input");var syncPhone=function(){canonicalPhone.value=visiblePhone&&visiblePhone.value||"";};var error=document.createElement("div");error.className="__artem-form-error";error.setAttribute("role","alert");error.setAttribute("aria-live","assertive");error.setAttribute("aria-hidden","true");form.insertBefore(error,form.firstChild);var email=form.querySelector('input[name="formParams[email]"]');var fullName=form.querySelector('input[name="formParams[full_name]"]');var consentPersonal=form.querySelector('input[type="checkbox"][id^="field-input-11904802"]');var consentMarketing=form.querySelector('input[type="checkbox"][id^="field-input-11904803"]');var clearError=function(){error.textContent="";error.setAttribute("aria-hidden","true");};var showError=function(message,control){error.textContent=message;error.setAttribute("aria-hidden","false");if(control&&control.focus){control.focus();}error.scrollIntoView({block:"nearest"});};var validate=function(event){syncPhone();var phoneDigits=canonicalPhone.value.replace(/\\D/g,"");var problem=!email||!email.value.trim()?[(email&&email.placeholder)||"Введите ваш эл. адрес",email]:!fullName||!fullName.value.trim()?[(fullName&&fullName.placeholder)||"Введите ваше имя",fullName]:!visiblePhone||phoneDigits.length<8?["Введите ваш телефон",visiblePhone]:!consentPersonal||!consentPersonal.checked?["Я соглашаюсь на обработку персональных данных в соответствии с политикой конфиденциальности",consentPersonal]:!consentMarketing||!consentMarketing.checked?["Я соглашаюсь на получение рекламных рассылок, звонков и сообщений",consentMarketing]:null;if(problem){event.preventDefault();event.stopImmediatePropagation();showError("Не заполнено поле "+problem[0],problem[1]);return false;}clearError();};if(visiblePhone){visiblePhone.addEventListener("input",syncPhone);visiblePhone.addEventListener("change",syncPhone);visiblePhone.addEventListener("blur",syncPhone);}[email,fullName,visiblePhone,consentPersonal,consentMarketing].forEach(function(control){if(control){control.addEventListener("input",clearError);control.addEventListener("change",clearError);}});form.addEventListener("submit",validate,true);syncPhone();form.action=window.location.origin+${JSON.stringify(`/api/manager-form/widget-submit/${input.sessionId}`)};window.parent.postMessage({type:"getcourse-manager-widget",status:"ready",sessionId:${JSON.stringify(input.sessionId)}} ,"*");}},0);});
</script>`;
